...
namelist /atmosphere_nml/ do_dynamics, p_split
...
! HII added for non-diluteness (needs to be passed to dynamics and physics)
...
!   endif
      ...
      call exo_init(isc,iec,jsc,jec, Atm(1)%npz, axes, Time, &
           ...
           Atm(1)%q(isc:iec,jsc:jec,1:Atm(1)%npz, 1:Atm(1)%ncnst), cold_start, non_dilute)
